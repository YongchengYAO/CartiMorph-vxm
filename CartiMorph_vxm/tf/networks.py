...
class TemplateCreation(ne.modelio.LoadableModel):
    ...
    # (added by YC Yao)
    # -----------------------------------
    def get_registration_model_tmp2img(self):
        """
        Returns a reconfigured model to predict only the atlas/template-to-image transform.
        """
        return keras.Model(self.inputs, self.references.pos_flow)
    ...
    # (added by YC Yao)
    # -----------------------------------
    def get_registration_model_img2tmp(self):
        """
        Returns a reconfigured model to predict only the image-to-atlas/template transform.
        """
        return keras.Model(self.inputs, self.references.neg_flow)

    # -----------------------------------
    # (this is the original class method)
    # (YC Yao) comment: this may be wrong
    # -----------------------------------
    # def register(self, src, trg):
    #     """
    #     Predicts the transform from src to trg tensors.
    ...
    # (added by YC Yao)
    # -----------------------------------
    def register_tmp2img(self, img):
        """
        Predicts the transform from the atlas/template to image.
        """
        return self.get_registration_model_tmp2img().predict(img)

    # -----------------------------------
    # (added by YC Yao)
    # -----------------------------------
    def register_img2tmp(self, img):
        """
        Predicts the transform from the image to atlas/template.
        """
        return self.get_registration_model_img2tmp().predict(img)
    ...
    # (added by YC Yao)
    # -----------------------------------
    def get_warpped_atlas(self, img, interp_method='linear', fill_value=None):
        """
        Predicts the transform from atlas/template to image, and apply it to the atlas/template
        """
        model_warpAtlas = keras.Model(self.inputs, self.references.vxm_model.references.y_source)
        return model_warpAtlas.predict(img)

    # -----------------------------------
    # (added by YC Yao)
    # -----------------------------------
    def get_warpped_img(self, img, interp_method='linear', fill_value=None):
        """
        Predicts the transform from image to atlas/template, and apply it to the image
        """
        model_warpImg = keras.Model(self.inputs, self.references.vxm_model.references.y_target)
        return model_warpImg.predict(img)
...
class ConditionalTemplateCreation(ne.modelio.LoadableModel):
    ...
    def __init__(self,
        ...
        # image input layers
        atlas_input = keras.Input((*inshape, src_feats), name='atlas_input')
        ...
        pheno_model = keras.models.Model([pheno_decoder_model.input, atlas_input], atlas_tensor)
